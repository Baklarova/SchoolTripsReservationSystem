namespace SchoolTripsReservationSystem.Infrastructure.Constants
{
    public static class DataConstants
    {
        public const int NameMaxLenght = 50;
        public const int NameMinLenght = 5;

        public const int SchoolAddressMaxLenght = 100;
        public const int SchoolAddressMinLenght = 10;

        public const int SchoolEikMaxLenght = 9;
        public const int SchoolEikMinLenght = 9;

        public const int ExcursionDescriptionMaxLenght = 500;
        public const int ExcursionDescriptionMinLenght = 20;

        public const int ExcursionDurationMaxNumber = 14;
        public const int ExcursionDurationMinNumber = 1;

        public const string ExcursionPriceMaxAmount = "9999";
        public const string ExcursionPriceMinAmount = "0";

        public const int TransportCapasityMaxNumber = 55;
        public const int TransportCapasityMinNumber = 14;

        public const int StudentsMaxPax = 50;
        public const int StudentsMixPax = 10;

        public const int AdultsMaxPax = 5;
        public const int AdultsMixPax = 0;

        public const string DateFormat = "dd.MM.yyyy HH:mm:ss";
    }
}
